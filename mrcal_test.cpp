...
//     bool do_optimize_intrinsics_core, bool
//     do_optimize_intrinsics_distortions, bool do_optimize_extrinsics, bool
//     do_optimize_frames, bool do_optimize_calobject_warp, bool
//     do_apply_regularization, bool do_apply_outlier_rejection, bool
//     Ncameras_intrinsics, bool Ncameras_extrinsics, bool Nframes, bool
//     Nobservations_board);
...
//       calobjectSize.height;                   // Object height, in # of
//       corners
...
//   int do_optimize_intrinsics_distortions = 1; // can skip intrinsics if we
//   want int do_optimize_extrinsics = 1;             // can skip extrinsics if
//   we want int do_optimize_frames = 1; int do_optimize_calobject_warp = 1; int
//   do_apply_regularization = 1; int do_apply_outlier_rejection = 1; // can
//   also skip
...
//   std::vector<double> intrinsics = {1200, 1200, cx, cy, 0, 0, 0, 0, 0, 0, 0,
//   0};
...
//   mrcal_point3_t points[0];     // Seems to always to be None for single
//   camera? int Ncameras_extrinsics = 0;  // Seems to always be zero for single
//   camera int Nframes =
...
//       do_apply_regularization, do_apply_outlier_rejection,
//       Ncameras_intrinsics, Ncameras_extrinsics, Nframes,
//       Nobservations_board);

//   int Nstate = mrcal_num_states(
//       Ncameras_intrinsics, Ncameras_extrinsics, Nframes, Npoints,
//       Npoints_fixed, Nobservations_board, problem_selections,
//       &mrcal_lensmodel);
...
//       Ncameras_intrinsics, Ncameras_extrinsics, Nframes, Npoints,
//       Npoints_fixed, problem_selections, &mrcal_lensmodel);
...
//       .point_min_range = point_min_range, .point_max_range =
//       point_max_range};
//   int verbose = 0;
...
//       calibration_object_width_n, calibration_object_height_n, verbose,
//       false);
...
//     do_optimize_intrinsics_distortions, bool do_optimize_extrinsics, bool
//     do_optimize_frames, bool do_optimize_calobject_warp, bool
//     do_apply_regularization, bool do_apply_outlier_rejection,
...
//     //   detect a corner, its coordinates are known less precisely, and we
//     use
//     //   that information to weight the errors appropriately later. We set
//     the
...
//   return mrcal_main(observations_board, frames_rt_toref, boardSize, 0.0254,
//   imagerSize).success;
// }
