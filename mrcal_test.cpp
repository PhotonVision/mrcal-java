...
#include <stdexcept>
using namespace cv;
mrcal_pose_t getSeedPose(const mrcal_point3_t *c_observations_board_pool, Size boardSize, Size imagerSize) {
  using namespace std;
...
void homography_test() {
  ...
  for (const auto & [key, value] : points) {
    std::vector<mrcal_point3_t> point_copy = value;
    if (value.size()) {
      auto ret = getSeedPose(point_copy.data(), {10, 10}, {1600, 896});
