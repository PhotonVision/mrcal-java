...
//       c_observations_point_triangulated[n_observations_point_triangulated];
  ...
  mrcal_calobject_warp_t calobject_warp[1] = {{0, 0}};
  ...
  // point breakdown is (x, y, weight, where weight<0 means ignored)
  std::vector<double> observations_board;
  observations_board.reserve(8*7*7*3);
  // Hack in known good observations
  ...
  std::vector<mrcal_point3_t> observations_point;
  observations_board.reserve(0);
  ...
  mrcal_point3_t *c_observations_point_pool = observations_point.data();

  // Copy from board/point pool above
  ...
  // hard-coded from seed of first solve
